...
var gameBoard = {
	...
	collisions:function(){
		...
		{
			var round1=this.actors[i];
			if(round1.visible)
			{
				for(var j=i+1;j<this.actors.length;j++)
				{
					var round2=this.actors[j];
					if(round2.visible)
					{
						if(this.hitCircle(round1,round2))
						{
							if(round1.energy<round2.energy)
							{
								round1.kill();
								round2.energy+=round1.energy;
							}
							else
							{
								round2.kill();
								round1.energy+=round2.energy;
							}
						}
					}
				}
...
function initialScreen() {
	...
	playerBall.energy=100;
	playerBall.actorClass='ball';
	
	var enemyBall=new CircleActor(200,200,15,true,'green',2,true);
	enemyBall.energy=80;
	enemyBall.angle=75;
	enemyBall.speed=5;
	...
	var enemyBall2=new CircleActor(350,400,15,true,'red',2,true);
	enemyBall2.energy=120;
	enemyBall2.angle=35;
	enemyBall2.speed=5;
	
	enemyBall2.bounceOnLeftWall=true;
	enemyBall2.bounceOnRightWall=true;
	enemyBall2.bounceOnTopWall=true;
	enemyBall2.bounceOnBottomWall=true;
	
	enemyBall2.actorClass = "ball";
	
	gameBoard.addActor(playerBall);
	gameBoard.addActor(enemyBall);
	gameBoard.addActor(enemyBall2);
	gameBoard.draw();
	...
	setInterval(function(){
		...
		if(gameBoard.hitCircle(playerBall, enemyBall)){
			if(playerBall.energy<enemyBall.energy){
				playerBall.kill();
			}
			else {
				enemyBall.kill();
				playerBall.energy+=enemyBall.energy;
			}
		}
		if(gameBoard.hitCircle(playerBall, enemyBall2)){
			if(playerBall.energy<enemyBall2.energy){
				playerBall.kill();
			}
			else {
				enemyBall2.kill();
				playerBall.energy+=enemyBall2.energy;
			}
		}
		if(gameBoard.hitCircle(enemyBall2, enemyBall)){
			if(enemyBall.energy<enemyBall2.energy){
				enemyBall.kill();
				enemyBall2.energy+=enemyBall.energy;
			}
			else {
				enemyBall2.kill();
				enemyBall.energy+=enemyBall2.energy;
			}
		}
		gameBoard.redraw();
	},30);
