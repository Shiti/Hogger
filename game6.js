					...
					if(r2.active && r1.active)
					{
						if(this.hitCircle(r1,r2))
						{
							if(r1.energy<r2.energy)
							{
								r1.kill();
								r2.energy+=r1.energy;
								r2.radius=Math.floor(r2.energy/6);
							}
							else
							{
								r2.kill();
								r1.energy+=r2.energy;
								r1.radius=Math.floor(r1.energy/6);
							}
						}
					}
				}
	...
	intervalId=setInterval(function(){
		...
		for(i in gameBoard.actors)
		{
			...
			{	
				...
				{
					actor.energy-=0.1;
					actor.radius-=(actor.radius)/550;
				}
				else if(actor.energy<=100)
				{
					actor.energy-=0.2;
					actor.radius-=(actor.radius)/500;

				}
				else
				{
					actor.energy-=0.4;
					actor.radius-=(actor.radius)/450;
